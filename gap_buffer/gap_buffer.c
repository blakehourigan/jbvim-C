...
GapBufferDoc *gap_init_doc(char *file_name) {
  ...
  // for line in the string, create a line buffer, and store a pointer to that
  // line buffer in the buffer, in order in which they appear in the document
...
void grow_buffer_doc(GapBufferDoc *doc) {
  ...
  printf("size: %d", cur_size);
  int des_size = cur_size + MAX_CAPACITY_INIT;
  printf("des size: %d", des_size);
